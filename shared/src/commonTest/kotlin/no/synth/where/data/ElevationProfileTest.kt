...
class ElevationProfileTest {
    ...
    fun carryForwardFillsNullAltitudes() {
        val p = assertNotNull(track(listOf(100.0, null, null, 140.0)).elevationProfileOrNull())
    ...
    @Test
    fun leadingNullAltitudesBackfillFromFirstReading() {
        // A null before the first real reading is filled forward from that reading, not zeroed,
        // so the leading segment doesn't show a phantom drop to 0.
        val p = assertNotNull(track(listOf(null, 100.0, 140.0)).elevationProfileOrNull())
        assertEquals(listOf(100.0, 100.0, 140.0), p.elevations)
        assertEquals(100.0, p.minEle)
        assertEquals(140.0, p.maxEle)
        assertEquals(40.0, p.gain)
    }

    @Test
    fun gainIgnoresDescents() {
        // 100 -> 200 (+100) -> 150 (drop, ignored) -> 180 (+30)
        val p = assertNotNull(track(listOf(100.0, 200.0, 150.0, 180.0)).elevationProfileOrNull())
    ...
    fun downsamplingCapsSamplesAndKeepsLastPoint() {
        ...
        val p = assertNotNull(track(altitudes).elevationProfileOrNull(maxSamples = 10))
    ...
    fun gainNotInflatedBySubDeadbandNoise() {
        ...
        val p = assertNotNull(track(altitudes).elevationProfileOrNull())
